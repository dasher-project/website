...
- **Source:** [`src/data/feature-status.yaml`](https://github.com/dasher-project/website/blob/astro-build/src/data/feature-status.yaml) in this repo.
...
The YAML is validated on every PR by
...
which also regenerates `src/data/feature-status.json` at build time. A malformed
entry **fails CI**.
...
2. Edit `src/data/feature-status.yaml` — find the feature, update the platform entry:
   ```yaml
   platforms:
...
5. Run `npm run validate:status` locally (it runs automatically in `prebuild` and CI).
...
> ☑ This changes a capability that users see on other platforms → I have opened/updated `website/src/data/feature-status.yaml` (PR: #...)
